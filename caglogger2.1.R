...
extract_cag_both_strands <- function(seq,
 ...
 fwd <- extract_cag_from_read(seq,     max_type_a, max_type_b, boundary_mode)
 rev <- extract_cag_from_read(rc(seq), max_type_a, max_type_b, boundary_mode)
...
for (file in fastq_files) {
  results[[file]] <- process_fastq_file(
    ...
    min_repeat_length = 0L,           # set to 35 for v1.2 behaviour
    max_type_a        = 5L,
    max_type_b        = 1L,
    chunk_size        = 50000L,
    allele_bw         = 1.5,
    output_dir        = "cag2.1results/"
  )
}
toc()